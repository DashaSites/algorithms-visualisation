...
describe("String testing", () => {
  ...
  it("checks if a string with odd number of symbols is being reversed correctly and animation works on each step", () => {
  ...
  it("checks if a string with even number of symbols is being reversed correctly and animation works on each step", () => {
    ...
    // Шаг-4: проверяю состояние
    cy.get("@outputArray").should("have.length", 4).each(($element) => {
      cy.wrap($element).find(circleSelector).as("circleElement");
      cy.get("@circleElement").invoke("attr", "class").then(classes => {
        expect(classes).to.contain("circle_modified");
      });
